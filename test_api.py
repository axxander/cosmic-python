...
def test_api_returns_allocation(add_stock, postgres_session):  # failing due to there being no batches in database?: doesn't look like in memory db is used: does it look at this in the test?
...
def test_allocations_are_persisted(add_stock):
    sku = random_sku()
    batch1, batch2 = random_batchref(name="1"), random_batchref(name="2")
    order1, order2 = random_orderid(name="1"), random_orderid(name="2")
    add_stock(
        [
...
def test_400_message_for_out_of_stock(add_stock):
    """Trying to allocated more units than we have in stock."""
